...
//        constexpr float DEG2RAD = PI / 180.0f;
        ...
        {
            return 23.45f * deg2rad(std::sin(2.0f * HGL_PI * (284 + day_of_year) / 365.0f));
        }
        ...
        inline float hour_angle(float hour, float longitude_deg)
        {
            ...
            return deg2rad((solar_time - 12.0f) * 15.0f);  // 小时角（弧度）
        }
        ...
        inline float solar_azimuth(float latitude_rad, float declination_rad, float hour_angle_rad, float elevation_rad)
        {
            ...
            if (hour_angle_rad > 0) azimuth = 2.0f * HGL_PI - azimuth; // 下午
            return azimuth;
        }
    ...
    void SkyInfo::SetByTimeOfDay(float hour, float minute, float second)
    {
        ...
        const float latitude_rad = deg2rad(this->latitude_deg);
        ...
        // 时间/仰角色调
        const Color4f day_sky_tint  (0.45f, 0.60f, 1.00f, 1.0f);   // 晴朗偏蓝
        const Color4f dusk_sky_tint (1.00f, 0.55f, 0.25f, 1.0f);   // 傍晚偏暖
        const Color4f night_sky_tint(0.08f, 0.10f, 0.18f, 1.0f);   // 夜晚深蓝

        // elev01 表示太阳仰角归一化，0..1
        const float elev01 = clamp01(elevation_rad / (HGL_PI * 0.5f));
        const float warm   = smoothstep(0.0f, 0.35f, 1.0f - elev01); // 仰角越低越暖

        // 环境相关：地表色调、湿度、气溶胶
        const Color4f env_tint = environment.SkyTint();
        const float humidity   = environment.Humidity();
        const float aerosol    = environment.Aerosol();
        ...
        if (is_day) {
            ...
            // 地平线附近加入地表反照影响：仰角越低，混入越多
            const float horizon_mix = smoothstep(0.0f, 0.4f, 1.0f - elev01); // 接近地平线
            const float ground_factor = clamp01(0.15f + 0.55f * humidity);   // 湿度越高越显著
            const Color4f time_tint = Lerp(day_sky_tint, dusk_sky_tint, warm);
            this->base_sky_color = Lerp(time_tint, env_tint, horizon_mix * ground_factor);

            // 光晕：湿度提升（更大更柔），气溶胶也会增强但偏黄
            ...
            // 简化的月亮方位（太阳对面）
            const float moon_azimuth = azimuth_rad + HGL_PI;
