...
enum class Prim
{
    ...
    SolidRectangles2D=0x100,            ///<实心矩形(并非原生支持。以画点形式在每个点的Position中传递Left,Top,Right,bottom。在Geometry Shader中转换为2个三角形。用于2D游戏或UI)
    WireRectangles2D,                   ///<线框矩形

    Billboard2DWay,                     ///<2D广告牌(以画线形式在第一个点中指定一个顶点，在第二个点中指定长宽高)
